...
final_result_as_list_plasmid = []
final_result_as_list_wo_plasmid = []
...
for isolate in final_result:
    ...
    for index, row in enumerate(final_result[isolate]):
        if index == 0:
            ...
            if row.contig == None or row.contig == "":
                null_plasmid_lines.append(curr_line)
                continue
    ...
    final_result_as_list_wo_plasmid.append(lines[0][0])

    # First line for plasmid
    if len(plasmid_lines) > 0:
        plasmid_first_line = lines[plasmid_lines[0]][0][:]          # Deep Copy
        plasmid_first_line[0] = lines[0][0][0]
        plasmid_first_line[1] = lines[0][0][1]
        plasmid_first_line[2] = lines[0][0][2]
        plasmid_first_line[3] = lines[0][0][3]
        final_result_as_list_plasmid.append(plasmid_first_line)
        used_lines.add(plasmid_lines[0])

    used_lines.add(0)

    # Step 2 -> Preparing plasmid data (another lines)

    # First we appemd all the null plasmid lines
    final_result_as_list_plasmid.extend(null_plasmid_lines)
    
    for contig_id in plasmid_lines:
        if contig_id not in used_lines and contig_id in lines:
            final_result_as_list_plasmid.extend(lines[contig_id])
            used_lines.add(contig_id)

    # Prepare the final lines for wo plasmid
    # FIrst we use the null lines
    final_result_as_list_wo_plasmid.extend(null_lines)

    for contig_id in lines:
        if contig_id not in used_lines:
            final_result_as_list_wo_plasmid.extend(lines[contig_id])
            used_lines.add(contig_id)


# Now We will use this object to create an excel file and save the result
pd_df = pd.DataFrame(final_result_as_list_plasmid, columns=column_names)
pd_df.to_excel("output/plasmid_result.xlsx", index=False)


# Now We will use this object to create an excel file and save the result
pd_df = pd.DataFrame(final_result_as_list_wo_plasmid, columns=column_names)
pd_df.to_excel("output/wo_plasmid_result.xlsx", index=False)
